...
function play() {
  ...
  let number = prompt('Hver er talan?');
  let guessCount= 1;
  let guess = parseGuess(number);
...
function getResponse(guess, correct){
  ...
  else if(parseInt(guess)===correct){
    return 'Rétt';
  }
  else if(Math.abs(correct-parseInt(guess)) < 5){
    return 'Mjög nálægt';
  }
  else if (Math.abs(correct-parseInt(guess)) < 10){
    return 'Nálægt';
  }
  else if (Math.abs(correct-parseInt(guess)) < 20){
    return 'Frekar langt frá';
  }
  else if (Math.abs(correct-parseInt(guess)) < 50){
    return 'Langt frá';
  }
  else{
